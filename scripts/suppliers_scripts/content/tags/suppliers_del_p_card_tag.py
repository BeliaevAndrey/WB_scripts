...
async def a_del_tag_delete(tag_id: int, color: str, name: str) -> tuple | dict:
    headers_dict = {
    ...
    async with aiohttp.ClientSession(headers=headers_dict) as session:
        async with session.delete(url=URL.format(id=tag_id), json=params) as response:
...
if __name__ == '__main__':
    ...
    tag_id = 473766
    pprint.pp(asyncio.run(a_del_tag_delete(tag_id=tag_id, **sample_req)))
